...
// https://github.com/UHH2/BstarToTW/blob/102X_v1/src/BstarToTWModules.cxx#L392
TopPtReweighting::TopPtReweighting(uhh2::Context& ctx,
  ...
  if(!weight_name_.empty()) h_weight_= ctx.get_handle<float>(weight_name);
  version_ = ctx.get("dataset_version", "");
  boost::algorithm::to_lower(version_);
  if(!ttgen_name_.empty()) h_ttbargen_ = ctx.get_handle<TTbarGen>(ttgen_name);

  if (syst_a == "up")        a_ *= 1.5;
  else if (syst_a == "down") a_ *= 0.5;

  if (syst_b == "up")        b_ *= 1.5;
  else if (syst_b == "down") b_ *= 0.5;
}
