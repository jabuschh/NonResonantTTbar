...
// #include <UHH2/NonResonantTTbar/include/utils.h>
...
bool BlindDataSelection::passes(const Event & event){
  ...
  if(!is_zprime_reconstructed_chi2) throw runtime_error("In NonResonantTTbarSelections.cxx:BlindDataSelection::passes(): The Zprime was never reconstructed via the chi2 method. This must be done before looking for the way it was reconstructed.");
...
bool ZprimeTopTagSelection::passes(const Event & event){
  ...
  bool is_zprime_reconstructed_chi2 = event.get(h_is_zprime_reconstructed_chi2);
  if(!is_zprime_reconstructed_chi2) throw runtime_error("In NonResonantTTbarSelections.cxx:ZprimeTopTagSelection::passes(): The Zprime was never reconstructed via the chi2 method. This must be done before looking for the way it was reconstructed.");
...
bool uhh2::Chi2Cut::passes(const uhh2::Event& event){
  ...
  // if(!is_zprime_reconstructed) throw runtime_error("In NonResonantTTbarSelections.cxx: Chi2Cut::passes: The Zprime was never reconstructed. Do this before trying to cut on its chi2.");
  bool pass = false;
